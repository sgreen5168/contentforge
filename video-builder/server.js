...
// ── FFmpeg path — detected at first use, not at startup ──────────────────────
let FFMPEG = null;
...
async function buildVideo(id, { topic, voice, duration, music, ratio }) {
  ...
  try {
    ...
    // Test Pexels key with a quick validation call first
    console.log('['+id+'] Validating Pexels API key...');
    try {
      const testR = await fetch('https://api.pexels.com/videos/search?query=nature&per_page=1',
        { headers: { Authorization: pKey } });
      console.log('['+id+'] Pexels validation: HTTP', testR.status);
      if (!testR.ok) throw new Error('HTTP '+testR.status);
      console.log('['+id+'] ✅ Pexels key valid');
    } catch(e) {
      throw new Error('Pexels API error: '+e.message+' — verify PEXELS_API_KEY in Railway Variables');
    }
    ...
    for (let i=0; i<keywords.length; i++) {
      try {
        ...
        if (!url) {
          for (const vid of (pd.videos||[])) {
            if (vid.video_files?.[0]?.link) { url=vid.video_files[0].link; break; }
          }
        }
        if (!url) { console.warn('['+id+'] No URL found for "'+keywords[i]+'"'); continue; }
        const cr = await fetch(url);
        if (!cr.ok) { console.warn('['+id+'] Download failed HTTP '+cr.status); continue; }
        const cb = Buffer.from(await cr.arrayBuffer());
        if (!cb||cb.length<5000) { console.warn('['+id+'] File too small: '+cb?.length+' bytes'); continue; }
        ...
        if (fs.existsSync(norm)&&fs.statSync(norm).size>1000) {
          clipPaths.push(norm);
          console.log('['+id+'] ✅ Clip '+(i+1)+' ready: '+Math.round(fs.statSync(norm).size/1024)+'KB');
        } else {
          console.warn('['+id+'] Norm output empty for clip '+(i+1));
        }
      } catch(e) { console.warn('['+id+'] Clip '+(i+1)+' error: '+e.message); }
      upd(id, { step:`🎬 Clip ${i+1}/${keywords.length} (${clipPaths.length} ready)` });
      await new Promise(r=>setTimeout(r,300));
    }
    console.log('['+id+'] Clips ready: '+clipPaths.length+'/'+keywords.length);
    if (clipPaths.length===0) throw new Error('No clips ready — check Railway Deploy Logs for details');
...
app.listen(PORT, async () => {
  console.log('🎬 Video Builder running on port', PORT);
  await getFFmpeg(); // warm up FFmpeg detection on startup
});
