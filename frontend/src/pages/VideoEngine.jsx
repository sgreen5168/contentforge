...
export default function VideoEngine() {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1100, fontFamily: 'inherit', background: C, minHeight: '100vh', borderRadius: 12 }}>
      {fatalError && (
        <div style={{ padding: 20, marginBottom: 16, background: 'rgba(226,75,74,.1)', border: '1px solid rgba(226,75,74,.2)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontSize: 12, color: '#F09595' }}>Error: {fatalError}</div>
          <button onClick={() => { setFatalError(null); setJob(null); setTab('generate'); }}
            style={{ padding: '6px 12px', borderRadius: 6, border: 'none', background: '#1D9E75', color: 'white', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>
            Reset
          </button>
        </div>
      )}
